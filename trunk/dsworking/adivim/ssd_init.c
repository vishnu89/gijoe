...
//sangseon revison
	...
	metadata->plane_meta[i].cold_active_page = blocks_to_skip*currdisk->params.pages_per_block + 512;
///////////////////////////////////////////////////////////////////////////////////////////////////
...
//sangseon revison
            ...
            metadata->cold_active_page = usable_blocks_per_plane * currdisk->params.pages_per_block + 512;
	    break;

        case PLANE_BLOCKS_PAIRWISE_STRIPE:
	    metadata->hot_active_page = (2 * usable_blocks_per_plane) * currdisk->params.pages_per_block;
	    metadata->cold_active_page = (2 * usable_blocks_per_plane) * currdisk->params.pages_per_block + 512;
            break;
...
//sangseon revison
            ...
            metadata->cold_active_page = (currdisk->params.planes_per_pkg * usable_blocks_per_plane) * currdisk->params.pages_per_block + 512;
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////// 
	    break;
...
//sangseon revison
    ...
    ASSERT(metadata->cold_active_page == metadata->plane_meta[0].cold_active_page);

    hot_active_block = metadata->hot_active_page / currdisk->params.pages_per_block;
    cold_active_block = metadata->cold_active_page / currdisk->params.pages_per_block;
...
//sangseon revision
	    ...
	    temp_bsn = bsn;
            ...
            metadata->block_usage[cold_active_block].bsn = temp_bsn ++;
        break;

        case SSD_COPY_BACK_ENABLE:
            for (i = 0; i < (unsigned int)currdisk->params.planes_per_pkg; i ++) {
                int plane_hot_active_block = SSD_PAGE_TO_BLOCK(metadata->plane_meta[i].hot_active_page, currdisk);
                int plane_cold_active_block = SSD_PAGE_TO_BLOCK(metadata->plane_meta[i].cold_active_page, currdisk);

                bitpos = ssd_block_to_bitpos(currdisk, plane_hot_active_block);
                ssd_set_bit(metadata->free_blocks, bitpos);
                bitpos = ssd_block_to_bitpos(currdisk, plane_cold_active_block);
                ssd_set_bit(metadata->free_blocks, bitpos);

		temp_bsn = bsn;

                metadata->block_usage[plane_hot_active_block].state = SSD_BLOCK_INUSE;
                metadata->block_usage[plane_hot_active_block].bsn = bsn ++;
                metadata->tot_free_blocks --;
                metadata->plane_meta[i].free_blocks --;

                metadata->block_usage[plane_cold_active_block].state = SSD_BLOCK_INUSE;
                metadata->block_usage[plane_cold_active_block].bsn = bsn ++;
                metadata->tot_free_blocks --;
                metadata->plane_meta[i].free_blocks --;
...
//////////////////////////////////////////////////////////////////////////////////
    ...
    //////////////////////////////////////////////////////////////////////////////
    // set the bsn for the ssd elemen
/////////////////////////////////////////////////////////////////////////////////////////
//sangseon revision
    metadata->hot_bsn = bsn;
    metadata->cold_bsn = bsn;
