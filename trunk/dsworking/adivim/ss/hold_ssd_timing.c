...
#else
double _ssd_write_page_osr(ssd_t *s, ssd_element_metadata *metadata, int lpn, int hc)
{
    double cost;
    unsigned int active_pagee;
    ...
    unsigned int pagepos_in_prev_blcok;
    unsigned int prev_plane;
    
    if(hc == 1)//hot
    {
	    active_page = metadata->hot_active_page;
    }
    else{
	    active_page = metadata->cold_active_page;
    }
    ...
    if(hc == 1)
    {
	    if(metadata->hot_lba_table[lpn] != -1) {
        	...
        	pagepos_in_prev_block = prev_page % s->params.pages_per_block;
        	prev_plane = metadata->block_usage[prev_block].plane_num;
	    ...
	    if(metadata->cold_lba_table[cbn] != -1) {
		    prev_block = metadata->cold_lba_table[cbn];
		    pagepos_in_prev_block = lpn % s->params.pages_per_block;
		    prev_plane = metadata->block_usage[prev_block].plane_num;
		    prev_page = prev_block + pagepos_in_prev_block;

		    if(metadata->block_usage[prev_block].page[pagepos_in_prev_block] != -1){
