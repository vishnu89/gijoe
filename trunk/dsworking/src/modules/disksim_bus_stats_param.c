...
static int
DISKSIM_BUS_STATS_PRINT_BUS_IDLE_STATS_depend (char *bv)
{
  return -1;
}

static void
DISKSIM_BUS_STATS_PRINT_BUS_IDLE_STATS_loader (int result, int i)
{
  if (!(RANGE (i, 0, 1)))
    {				// foo 
    }
...
static int
DISKSIM_BUS_STATS_PRINT_BUS_ARBWAIT_STATS_depend (char *bv)
{
  return -1;
}

static void
DISKSIM_BUS_STATS_PRINT_BUS_ARBWAIT_STATS_loader (int result, int i)
{
  if (!(RANGE (i, 0, 1)))
    {				// foo 
    }
