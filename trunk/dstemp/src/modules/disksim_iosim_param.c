...
static int
DISKSIM_IOSIM_IO_TRACE_TIME_SCALE_depend (char *bv)
{
  return -1;
}

static void
DISKSIM_IOSIM_IO_TRACE_TIME_SCALE_loader (int result, double d)
{
  if (ioscale < 0.0)
    {
      if (ioscale > -1.0)
...
static int
DISKSIM_IOSIM_IO_MAPPINGS_depend (char *bv)
{
  return -1;
}

static void
DISKSIM_IOSIM_IO_MAPPINGS_loader (int result, struct lp_list *l)
{
  if (!(iosim_load_mappings (l)))
    {				// foo 
    }
